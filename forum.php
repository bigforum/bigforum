...
while($thd = mysql_fetch_object($them_dat))
{
  ...
  {
    echo "<tr><td width=3%>$close</td><td width=50%><b>$wich <a href=thread.php?id=$thd->id>$thd->tit</a></b></td><td width=20%><span style=\"cursor: pointer;\" onclick=\"window.location.href='profil.php?id=$vd->id'\">$thd->verfas</span><td width=10%>$zahl</td></tr>";
  }
  else
    echo "<tr><td width=3%>$close</td><td width=50%>$wich <a href=thread.php?id=$thd->id>$thd->tit</a></td><td width=20%><span style=\"cursor: pointer;\" onclick=\"window.location.href='profil.php?id=$vd->id'\">$thd->verfas</span><td width=10%>$zahl</td></tr>";
}
echo "</table>";
if($topics == "0")
{
